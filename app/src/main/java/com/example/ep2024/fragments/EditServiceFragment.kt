...
import com.example.ep2024.models.EventType
import com.example.ep2024.models.Service
import com.example.ep2024.utils.service.ChangeEntry
import com.example.ep2024.utils.service.ConfirmationMethod
import com.example.ep2024.utils.service.ServiceChangeHistory
...
class EditServiceFragment : Fragment() {
    ...
    private fun saveServiceChanges() {
        ...
        if (selectedEventTypes.isEmpty()) {
            Toast.makeText(requireContext(), "Пожалуйста, выберите хотя бы один тип события.", Toast.LENGTH_SHORT).show()
            return
        }
    ...
    private fun confirmAndDeleteService() {
        ...
        if (hasFutureBookings(service.id)) {
            Toast.makeText(requireContext(), "Невозможно удалить услугу, имеются будущие бронирования.", Toast.LENGTH_LONG).show()
            return
        }
